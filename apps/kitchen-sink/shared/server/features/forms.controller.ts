...
export class FormsController {
  constructor(
    ...
    @Inject(KITCHEN_SINK_OPTIONS) private readonly options: KitchenSinkOptions,
  ) {}
  ...
  image(@Param('id') id: string) {
    const upload = this.gallery.find(Number(id))
    if (!upload) throw new NotFoundException()
    return new StreamableFile(upload.bytes, { type: upload.type, length: upload.size })
  }

  // The POST that receives the file is platform code: each app's upload.controller.ts.
