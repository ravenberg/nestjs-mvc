...
const derive = (password: string, salt: Buffer, params: typeof PARAMS): Promise<Buffer> =>
...
const DUMMY = ['scrypt', PARAMS.N, PARAMS.r, PARAMS.p, Buffer.alloc(16).toString('base64'), Buffer.alloc(KEY_LENGTH).toString('base64')].join('$')
...
export async function verifyPassword(password: string, stored: string | null | undefined): Promise<boolean> {
  ...
  return stored !== null && stored !== undefined && want.length === key.length && timingSafeEqual(want, key)
}
