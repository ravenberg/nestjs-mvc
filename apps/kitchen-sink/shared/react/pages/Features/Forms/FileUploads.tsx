...
/** How the server receives the file: the one part of this page that differs per platform. */
const receiving = {
  express: (
    <>
      <code className="rounded bg-slate-100 px-1">@UseInterceptors(FileInterceptor('avatar'))</code> and{' '}
      <code className="rounded bg-slate-100 px-1">@UploadedFile()</code> (Nest's own tooling, Express only) hand the handler the file, and the
      text fields arrive in <code className="rounded bg-slate-100 px-1">@Body()</code> as usual.
    </>
  ),
  fastify: (
    <>
      <code className="rounded bg-slate-100 px-1">@fastify/multipart</code> parses it, and the handler reads the file and the text fields
      from <code className="rounded bg-slate-100 px-1">req.parts()</code>; Nest's <code className="rounded bg-slate-100 px-1">FileInterceptor</code>{' '}
      is Express only.
    </>
  ),
}

export default function FileUploads({ uploads, platform }: { uploads: Upload[]; platform: keyof typeof receiving }) {
      ...
      <div className="grid max-w-5xl gap-4 md:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
        ...
        >
          <p className="text-slate-600">
            ...
            <code className="rounded bg-slate-100 px-1">multipart/form-data</code>. On the server, {receiving[platform]} The image
            is kept in memory and served back by a plain route.
          </p>
