...
test.describe('Forms', () => {
  ...
  test('File uploads: a file over the 2 MB limit is refused with 413, on every platform', async ({ page, context }) => {
    // Express gets the 413 from FileInterceptor's limit; Fastify's plugin error would be a 500 unless the handler maps it.
    await page.goto('/features/forms/file-uploads')
    const token = (await context.cookies()).find((cookie) => cookie.name === 'XSRF-TOKEN')!.value
    const response = await page.request.post('/features/forms/file-uploads', {
      headers: { 'X-XSRF-TOKEN': decodeURIComponent(token) },
      multipart: {
        caption: 'too big',
        avatar: { name: 'big.png', mimeType: 'image/png', buffer: Buffer.alloc(3 * 1024 * 1024) },
      },
    })
